...
#group, train_X, train_Y, val_X, val_Y = stratified_train_test_group_kfold_multilabel(train_val_X, train_val_Y,label_dict, train_val_groups, n_splits=5, test_fold=0)
...
model_path = 'CNNI_AFM_res152_fine2-3-4-5label-train_val_total'
...
PATH = os.path.join(base_path, 'Models/CNNI_AFM_res152_fine2-3-4label/Fold2.pth') # pretrained model to be further fine-tuned


class_vary = 4

multilabel_train_folds(Batch_size, LR, DROP, criterion, n_train, n_val, train_val_X, train_val_Y, label_dict,train_val_groups, class_vary, base_path, model_path, pretrain_model,model_from_pretrain, PATH=PATH, less_volume = True, sequence=True, epochs=60, n_splits=5)
