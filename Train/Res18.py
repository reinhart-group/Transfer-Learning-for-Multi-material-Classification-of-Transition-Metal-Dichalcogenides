...
pretrain_model = models.resnet18(weights=ResNet18_Weights.IMAGENET1K_V1)

drop = 0
def model_from_pretrain(pretrain_model, drop):
    pretrain_model.fc = nn.Sequential(nn.Linear(512, 100), #150, 1
                                     nn.ReLU(),
                                     nn.Dropout(p=drop),
                                     nn.Linear(100, 5),
                                     )
...
model_path = 'CNNI_Res18_fineclass5.pth'
...
Folds_acc, Folds_f1score, Folds_conf = multiclass_inference_folds(pretrain_model, model_from_pretrain,cnn_class_metrics, base_path, model_path, n_train, n_val, train_val_X, train_val_Y, test_X, test_Y, train_val_groups, class_vary, batch_size = 16,  less_volume=False, sequence=False, n_splits=5)
